...
* **AI-Powered Background Removal**: Integrated with `rembg` to clean input photos automatically without manual masking.
...
```bash
pip install Pillow rembg psd-tools pytoshop packbits numpy tqdm
```
...
## Python Script (`process_501st_batch.py`)
...
def get_head_center_x(subject_img: Image.Image) -> float:
    ...
    (top 35% of subject cutout) to ensure precise facial centering.
    """
...
def composite_photo(subject_img: Image.Image, bg_img: Image.Image, frame_overlay: Image.Image, fit_mode: str = "waist"):
    ...
    # Align specifically by head center rather than overall bounding box
